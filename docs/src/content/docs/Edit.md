...
When the data provider returns no record, `<EditGuesser>` renders an empty state by default. You can override it using the `empty` prop (including `empty={null}` to render nothing).
