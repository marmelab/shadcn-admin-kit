...
You can find more advanced examples of `<InfiniteList>` usage in the [demo](https://marmelab.com/shadcn-admin-kit/demo/).
...
| `pagination` | Optional | `ReactNode` | `<InfiniteListPagination />` | Custom pagination component |
...
`<InfiniteList>` itself doesn't render the list of records. It delegates this task to its children components. These children components grab the `data` from the [`ListContext`](https://marmelab.com/ra-core/uselistcontext/.html) and render them on screen.
...
Alternatively to `children`, you can pass a `render` prop to `<InfiniteList>`. It will receive the [`ListContext`](https://marmelab.com/ra-core/uselistcontext/.html#return-value) as its argument, and should return a React node. This allows to inline the render logic for the list page.
...
:::note
When receiving a `render` prop, the `<InfiniteList>` component will ignore the `children` prop.
:::
...
export const PostList = () => (
    <InfiniteList actions={<MyActions />}> 
        ...
    </InfiniteList>
);
```
...
```tsx
export const PostList = () => (
    <InfiniteList title="List of posts">
        ...
    </InfiniteList>
);
```
...
```tsx
export const PostList = () => (
    <InfiniteList sort={{ field: 'published_at', order: 'DESC' }}>
        ...
    </InfiniteList>
);
```
...
// in src/posts.js
export const PostList = () => (
    <InfiniteList filter={{ is_published: true }}>
        ...
    </InfiniteList>
);
```
...
Filter Inputs are regular inputs. `<InfiniteList>` hides them all by default, except those that have the `alwaysOn` prop. Note that inputs with `alwaysOn` don't accept `defaultValue`. You have to use the `filterDefaultValues` for those.
...
export const PostList = () => (
    <InfiniteList filters={postFilters} filterDefaultValues={{ is_published: true }}>
        ...
    </InfiniteList>
);
```
...
const PostList = () => (
    <InfiniteList exporter={exporter}>
        ...
    <InfiniteList>
)
```
...
:::tip
The `<ExportButton>` limits the main request to the `dataProvider` to 1,000 records. If you want to increase or decrease this limit, pass a `maxResults` prop to the `<ExportButton>` in a custom `<InfiniteListActions>` component.
:::
...
`<InfiniteList>` accepts a `queryOptions` prop to pass [query options](https://marmelab.com/ra-core/actions/#query-options) to the react-query client. Check react-query's [`useQuery` documentation](https://tanstack.com/query/v5/docs/react/reference/useQuery) for the list of available options.
...
const PostList = () => (
    <InfiniteList queryOptions={{ meta: { foo: 'bar' } }}>
        ...
    <InfiniteList>
);
```
...
const PostList = () => {
    ...
    return (
        <InfiniteList queryOptions={{ onError }}>
            ...
        <InfiniteList>
    );
}
...
You can use `<InfiniteListGuesser>` to quickly bootstrap a List view on top of an existing API, without adding the fields one by one.
...
Just like `<InfiniteList>`, `<ListGuesser>` fetches the data. It then analyzes the response, and guesses the fields it should use to display a basic `<DataTable>` with the data. It also dumps the components it has guessed in the console, so you can copy it into your own code.
...
If you want to subscribe to live updates on the list of records, add [the `<InfiniteListLiveUpdate>` component](https://marmelab.com/ra-core/listliveupdate/) in your `<List>` children.
...
:::note
If you need to set the list parameters to render a list of records *related to another record*, there are better components than `<InfiniteList>` for that. Check out the following components, specialized in fetching and displaying a list of related records:
...
Besides fetching a list of records from the data provider, `<InfiniteList>` renders the default list page layout (title, buttons, filters, a `<Card>`, pagination) and its children. If you need a custom list layout, you may prefer [the `<ListBase>` component](https://marmelab.com/ra-core/listbase/), which only renders its children in a [`ListContext`](https://marmelab.com/ra-core/uselistcontext/.html).
...
The previous example leverages [`<WithListContext>`](https://marmelab.com/ra-core/withlistcontext/) to grab the data that `<InfiniteListBase>` stores in the `ListContext`.

If you don't need the `ListContext`, you can use [the `useListController` hook](https://marmelab.com/ra-core/uselistcontroller/), which does the same data fetching as `<InfiniteListBase>` but lets you render the content.
...
If your `authProvider` implements [Access Control](https://marmelab.com/ra-core/permissions/#access-control), `<InfiniteList>`  will only render if the user can access the resource with the "list" action.
