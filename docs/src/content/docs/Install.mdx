...
Every command below passes `--base base --preset vega`. Keep both flags: the shadcn CLI can generate its components on top of either Base UI or Radix primitives, and this version of `shadcn-admin-kit` is built on the Base UI ones, in the `vega` preset. Together they produce the `base-vega` style the kit's own `components.json` uses; without the preset flag the CLI stops to ask which one you want. The command otherwise asks no questions, and signs off with a reminder to wrap your app in a `TooltipProvider` that you can ignore, since the components that use tooltips provide their own.
