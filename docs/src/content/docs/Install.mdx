...
The template leaves `src/App.tsx` rendering `<ComponentExample />`, a showcase of the shadcn/ui components it just installed. Replace all its content by the following to serve your new `shadcn-admin-kit` application.
...
Shadcn Admin Kit reads `process.env.NODE_ENV`, so `node` must be among the TypeScript types your app loads. Add it to the `types` array of `tsconfig.app.json`, or `npm run build` fails with `Cannot find name 'process'`.
...
Shadcn Admin Kit reads `process.env.NODE_ENV`, so `node` must be among the TypeScript types your app loads. Add it to the `types` array of `tsconfig.json`, or `npm run build` fails with `Cannot find name 'process'`.
