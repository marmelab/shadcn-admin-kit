...
Shadcn Admin Kit reads `process.env.NODE_ENV`, so add `node` to the `types` of your `tsconfig.app.json`. Without it, `npm run build` fails with `Cannot find name 'process'`.
...
Add `node` to the `types` of your `tsconfig.json`, or `npm run build` fails with `Cannot find name 'process'`.
