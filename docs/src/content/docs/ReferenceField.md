...
- On long lists (30+ rows), avoid children that update state in a layout effect, such as an `<Avatar>` with an `<AvatarImage>`. Each row settling its own fetch triggers its own React update, and a commit-phase update in a child stops React from batching them, so they nest and exceed the limit of 50, throwing `Maximum update depth exceeded`. `<ReferenceField>` reaches that state reliably because ra-core's `useGetManyAggregate` settles all the pending calls together, but the same cascade is possible without it. Paginate or cap the list. See the [Radix UI to Base UI migration guide](./radix-to-base-ui-migration.md) for the measured threshold.
