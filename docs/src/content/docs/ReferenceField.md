...
- On long lists (30+ rows), avoid children that update state in a layout effect, such as an `<Avatar>` with an `<AvatarImage>`. `<ReferenceField>` fetches through ra-core's `useGetManyAggregate`, which resolves its pending calls one at a time; a commit-phase update in a child stops React from batching them, so they nest and exceed the limit of 50, throwing `Maximum update depth exceeded`. Paginate or cap the list. See the [Radix UI to Base UI migration guide](./radix-to-base-ui-migration.md) for the measured threshold.
