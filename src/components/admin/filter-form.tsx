...
export const FilterFormBase = (props: FilterFormBaseProps) => {
  ...
  useEffect(() => {
    if (!filters) return;
    filters.forEach((filter: React.ReactElement<any>) => {
  ...
  const getShownFilters = () => {
    ...
    return filters.filter((filterElement: React.ReactElement<any>) => {
  ...
  const handleHide = useCallback(
    (event: any) => hideFilter(event.currentTarget.dataset.key),
    [hideFilter]
  );

  return (
    <>
      {getShownFilters().map((filterElement: React.ReactElement<any>) => (
        <FilterFormInput
